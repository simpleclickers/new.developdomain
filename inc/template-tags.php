...
function wds_acme_post_date( $args = [] ) {
	...
	// Set defaults.
	$defaults = [
		'date_text'   => esc_html__( 'Posted on', 'oneortwo-ltd' ),
...
function wds_acme_post_author( $args = [] ) {
	...
	// Set defaults.
	$defaults = [
		'author_text' => esc_html__( 'by', 'oneortwo-ltd' ),
	];
...
function wds_acme_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		$categories_list = get_the_category_list( esc_attr__( ', ', 'oneortwo-ltd' ) );
		if ( $categories_list && wds_acme_categorized_blog() ) {

			/* translators: the post category */
			printf( '<span class="cat-links">' . esc_attr__( 'Posted in %1$s', 'oneortwo-ltd' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_attr__( ', ', 'oneortwo-ltd' ) );
		if ( $tags_list ) {

			/* translators: the post tags */
			printf( '<span class="tags-links">' . esc_attr__( 'Tagged %1$s', 'oneortwo-ltd' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
		}
	}

	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( esc_attr__( 'Leave a comment', 'oneortwo-ltd' ), esc_attr__( '1 Comment', 'oneortwo-ltd' ), esc_attr__( '% Comments', 'oneortwo-ltd' ) );
		echo '</span>';
	}
	...
	edit_post_link(
		sprintf(
			...
			esc_html__( 'Edit %s', 'oneortwo-ltd' ),
...
function wds_acme_get_svg( $args = [] ) {
	...
	if ( empty( $args ) ) {
		return esc_attr__( 'Please define default parameters in the form of an array.', 'oneortwo-ltd' );
	}

	// Define an icon.
	if ( false === array_key_exists( 'icon', $args ) ) {
		return esc_attr__( 'Please define an SVG icon filename.', 'oneortwo-ltd' );
	}
			...
			if ( ! empty( $network_url ) ) :
				...
				<li class="social-icon <?php echo esc_attr( $network ); ?> mr-2">
					<a href="<?php echo esc_url( $network_url ); ?>">
						...
						printf( esc_attr__( 'Link to %s', 'oneortwo-ltd' ), ucwords( esc_html( $network ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
						?>
						</span>
					</a>
				</li><!-- .social-icon -->
...
function wds_acme_display_numeric_pagination( $args = [], $query = null ) {
	...
	<nav class="container pagination-container" aria-label="<?php esc_attr_e( 'numeric pagination', 'oneortwo-ltd' ); ?>">
...
function wds_acme_display_mobile_menu() {
	...
	<nav class="off-canvas-container" aria-label="<?php esc_attr_e( 'Mobile Menu', 'oneortwo-ltd' ); ?>" aria-hidden="true" tabindex="-1">
		<?php
		// Mobile menu args.
		$mobile_args = [
